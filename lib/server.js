...
server.init = () => {
    ...
    server.httpServer.listen(config.httpPort, () => {
        console.log('\x1b[36m%s\x1b[0m',`HTTP Server: http://localhost:${config.httpPort} mode: ${config.envName}`);
    })

    // Start the HTTPS server
    server.httpsServer.listen(config.httpsPort, () => {
        console.log('\x1b[35m%s\x1b[0m',`SSL Server: https://localhost:${config.httpsPort} mode: ${config.envName}`)
    })
}
